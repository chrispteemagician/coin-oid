exports.handler = async (event) => {
    ...
    if (!code) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Missing code parameter' }) };
    ...
    try {
        ...
        if (!identityRes.ok) return { statusCode: 401, headers, body: JSON.stringify({ error: 'Identity fetch failed' }) };
        ...
        if (!isPro) {
            ...
            if (ownCampaigns.length > 0) { tier = 'founder'; isPro = true; }
        }

        return { statusCode: 200, headers, body: JSON.stringify({ isPro, tier }) };
